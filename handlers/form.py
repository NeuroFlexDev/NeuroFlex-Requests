...
from storage import (save_request, save_temp_attachment, link_user_files_to_request,
                     FILES_ROOT, draft_save, draft_load, draft_delete)
from sheets import append_row_safe
from i18n import t
from keyboards import confirm_keyboard, contact_keyboard, reqtype_inline, budget_inline, nav_inline, edit_fields_keyboard
...
def _prev_state(cur: Form) -> Form:
    ...
    if cur in (Form.AI_DATA, Form.AI_DATASET): return Form.REQ_TYPE
    if cur in (Form.WEB_AUTH, Form.WEB_INTEGRATIONS): return Form.REQ_TYPE
...
async def _handle_nav(update: Update, context: ContextTypes.DEFAULT_TYPE, state_on_skip: Form):
    ...
    if action == "back":
        prev = _prev_state(context.user_data["_state"])
        context.user_data["_state"] = prev
        await q.answer(); await q.edit_message_text(t(lang,"back_step"))
        # отрисуем подсказку предыдущего шага
        return await ask_for_state(q.message, context, prev)
    if action == "skip":
        await q.answer(t(lang,"skipped"))
        return state_on_skip
    if action == "save":
...
async def ask_for_state(message, context, state: Form):
    lang = context.user_data.get("lang","ru")
    if state == Form.NAME: await message.reply_text(t(lang,"ask_name"), reply_markup=nav_inline(lang)); return Form.NAME
    if state == Form.COMPANY: await message.reply_text(t(lang,"ask_company"), reply_markup=nav_inline(lang)); return Form.COMPANY
    if state == Form.EMAIL: await message.reply_text(t(lang,"ask_email"), reply_markup=nav_inline(lang)); return Form.EMAIL
    if state == Form.CONTACT: await message.reply_text(t(lang,"ask_contact"), reply_markup=contact_keyboard(lang)); return Form.CONTACT
    if state == Form.REQ_TYPE:
        await message.reply_text(t(lang,"ask_req_type"))
        await message.reply_text(t(lang,"choose_req_type"), reply_markup=reqtype_inline(lang)); return Form.REQ_TYPE
    if state == Form.DESC: await message.reply_text(t(lang,"ask_description"), reply_markup=nav_inline(lang)); return Form.DESC
    if state == Form.FILES: await message.reply_text(t(lang,"ask_files")+" "+t(lang,"hint_files"), reply_markup=nav_inline(lang)); return Form.FILES
    if state == Form.BUDGET: await message.reply_text(t(lang,"ask_budget"), reply_markup=budget_inline(lang)); return Form.BUDGET
    return Form.CONFIRM
...
async def name_h(update, context):
    ...
    await update.message.reply_text(t(lang,"ask_company"), reply_markup=nav_inline(lang)); return Form.COMPANY

async def company_h(update, context):
    context.user_data["_state"]=Form.COMPANY
    context.user_data["company"]=update.message.text
    lang=context.user_data["lang"]
    await update.message.reply_text(t(lang,"ask_email"), reply_markup=nav_inline(lang)); return Form.EMAIL
...
async def email_h(update, context):
    ...
    if not validate_email(val):
        await update.message.reply_text(t(lang,"bad_email"), reply_markup=nav_inline(lang)); return Form.EMAIL
    context.user_data["email"]=val
    await update.message.reply_text(t(lang,"ask_contact"), reply_markup=contact_keyboard(lang)); return Form.CONTACT
...
async def contact_h(update, context):
    ...
    if not validate_phone(phone):
        await update.message.reply_text(t(lang,"bad_phone"), reply_markup=contact_keyboard(lang)); return Form.CONTACT
    context.user_data["contact"]=phone
    await update.message.reply_text(t(lang,"ask_req_type"))
...
async def reqtype_text(update, context):
    ...
    lang=context.user_data["lang"]
    await update.message.reply_text(t(lang,"ask_description"), reply_markup=nav_inline(lang)); return Form.DESC
...
async def reqtype_cb(update, context):
    ...
    # ветвление
    if value == "AI":
        await q.edit_message_text(t(lang,"ai_data")); return Form.AI_DATA
    if value == "WEB":
        await q.edit_message_text(t(lang,"web_auth")); return Form.WEB_AUTH
    # без доп.ветвления
    await q.edit_message_text(t(lang,"ask_description")); return Form.DESC

async def ai_data_h(update, context):
    context.user_data["_state"]=Form.AI_DATA
    context.user_data["ai_data"]=update.message.text
    lang=context.user_data["lang"]
    await update.message.reply_text(t(lang,"ai_dataset"), reply_markup=nav_inline(lang)); return Form.AI_DATASET
...
async def web_auth_h(update, context):
    ...
    lang=context.user_data["lang"]
    await update.message.reply_text(t(lang,"web_integrations"), reply_markup=nav_inline(lang)); return Form.WEB_INTEGRATIONS
...
async def desc_h(update, context):
    ...
    lang=context.user_data["lang"]
    await update.message.reply_text(t(lang,"ask_files")+" "+t(lang,"hint_files"), reply_markup=nav_inline(lang)); return Form.FILES

async def files_h(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["_state"]=Form.FILES
    lang = context.user_data["lang"]; uid = update.effective_user.id
    # документы
    if update.message.document:
        doc = update.message.document
        # простая фильтрация форматов
        ...
        if doc.mime_type and doc.mime_type not in allowed:
            await update.message.reply_text(t(lang,"file_reject")); return Form.FILES
        tg_file = await doc.get_file()
        dest = (FILES_ROOT / f"user_{uid}"); dest.mkdir(parents=True, exist_ok=True)
        local_path = dest / f"{doc.file_unique_id}_{doc.file_name or 'file'}"
        await tg_file.download_to_drive(custom_path=str(local_path))
        save_temp_attachment(uid, file_id=doc.file_id, file_name=doc.file_name, mime_type=doc.mime_type, file_size=doc.file_size, local_path=str(local_path))
        await update.message.reply_text(t(lang,"file_saved")); return Form.FILES
    # фото
    if update.message.photo:
        ...
        await tg_file.download_to_drive(custom_path=str(local_path))
        save_temp_attachment(uid, file_id=photo.file_id, file_name=f"{photo.file_unique_id}.jpg", mime_type="image/jpeg", file_size=photo.file_size or 0, local_path=str(local_path))
        await update.message.reply_text(t(lang,"file_saved")); return Form.FILES
    # просто текст (игнор) — подсказываем
    await update.message.reply_text(t(lang,"hint_files")); return Form.FILES
...
async def budget_cb(update, context):
    ...
    lang=context.user_data["lang"]; await q.answer("OK")
...
async def edit_cb(update, context):
    ...
    await q.answer()
    if field == "done":
        await q.edit_message_text(t(lang,"confirm_hint")); return Form.CONFIRM
    ...
    await q.edit_message_text(t(lang,prompt)); return Form.EDIT_FIELD
...
async def confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; _, val = q.data.split(":")
    lang = context.user_data.get("lang","ru")
    if val == "yes":
        data = dict(context.user_data)
        # pydantic валидация финальная
        model = RequestModel(
            name=data["name"], company=data.get("company"), email=data["email"], contact=data["contact"],
            req_type=data["req_type"], description=data["description"], budget=data.get("budget"),
...
def setup(app):
    conv = ConversationHandler(
        ...
        states={
            ...
            Form.AI_DATA: [MessageHandler(filters.TEXT & ~filters.COMMAND, ai_data_h),
                        CallbackQueryHandler(nav_cb, pattern=r"^nav:(back|skip|save)$")],
            Form.AI_DATASET: [MessageHandler(filters.TEXT & ~filters.COMMAND, ai_dataset_h),
                        CallbackQueryHandler(nav_cb, pattern=r"^nav:(back|skip|save)$")],
            Form.WEB_AUTH: [MessageHandler(filters.TEXT & ~filters.COMMAND, web_auth_h),
                        CallbackQueryHandler(nav_cb, pattern=r"^nav:(back|skip|save)$")],
            Form.WEB_INTEGRATIONS: [MessageHandler(filters.TEXT & ~filters.COMMAND, web_integrations_h),
                        CallbackQueryHandler(nav_cb, pattern=r"^nav:(back|skip|save)$")],
            ...
            Form.EDIT_FIELD: [MessageHandler(filters.TEXT & ~filters.COMMAND, edit_field_h)]
        },
